...
main (int argc, char *argv[])
{
  ...
  std::string scenario = "Simple";
   // Create simulation helper  //example-tutorial  simple-GEO
  ...
  Singleton<SatEnvVariables>::Get ()->SetOutputVariables ("sat-mobility", "GEO", true);
  ...
  Ptr<SatListPositionAllocator> enbPositionAlloc = CreateObject<SatListPositionAllocator> ();
  for (uint16_t i = 0; i < numberOfEnbs; i++)
    {
      ...
      //GEO 
      enbPositionAlloc->Add (GeoCoordinate (latitude, longitude+ i *2, 0));
  ...
  enbMobility.SetMobilityModel ("ns3::SatConstantPositionMobilityModel");
  enbMobility.SetPositionAllocator (enbPositionAlloc);
  enbMobility.Install (enbNodes);

  for ( uint32_t i = 0; i < enbNodes.GetN (); i++ )
    {
      Ptr<SatMobilityModel> model = enbNodes.Get (i)->GetObject<SatMobilityModel> ();
      GeoCoordinate pos = model->GetGeoPosition ();

      // check that position is in limit
      std::cout <<" pos.GetLatitude ()  "<< pos.GetLatitude ()<<std::endl;
      std::cout <<" pos.GetLongitude()  "<< pos.GetLongitude ()<<std::endl;
      std::cout <<" pos.GetAltitude ()  "<< pos.GetAltitude ()<<std::endl;
    }
  ...
  Ptr<SatListPositionAllocator> uePositionAlloc = CreateObject<SatListPositionAllocator> ();
  uePositionAlloc->Add (GeoCoordinate (latitude, longitude, 0));
  ueMobility.SetPositionAllocator (uePositionAlloc);
  ueMobility.Install (ueNodes);
  //ueNodes.Get (0)->GetObject<MobilityModel> ()->SetPosition (Vector (0, yForUe, 0));
  ueNodes.Get (0)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (speed, 0, 0));
  
  //create satellite netDevice
  // Ptr<SatSimpleNetDevice> dev0 = CreateObject<SatSimpleNetDevice> ();
  // Ptr<SatSimpleNetDevice> dev1 = CreateObject<SatSimpleNetDevice> ();
  ...
  // channel->SetFreeSpaceLoss (Fsl);

  // //channel attach netDevice
  // dev0->SetChannel (channel);
  // dev1->SetChannel (channel);

  //netDevice add to a node
  //n0->AddDevice (dev0);
  //n1->AddDevice (dev1);
  ...
  p2ph.EnablePcapAll("geo-x2-handover-measures");
